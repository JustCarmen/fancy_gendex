...
class fancy_gendex_WT_Module extends WT_Module implements WT_Module_Config {
	...
	private function get_gendex_content($tree, $indis) {
		...
		foreach($indis as $indi) {
			...
			if($record && $record->canShowName(WT_PRIV_PUBLIC)) {
				$content.=$record->getXref().'&ged='.$tree->tree_name.'|' .$indi['SURNAME'].'|'.$indi['GIVN'].' /'.$indi['SURNAME'].'/|'.$this->print_date('BIRT', $xref, $tree).'|'.$record->getBirthPlace().'|'.$this->print_date('DEAT', $xref, $tree).'|'.$record->getDeathPlace().'|'.PHP_EOL;
			}
		}
		return $content;
	}
	...
	private function print_date($fact, $xref, $tree) {
		$row=
			WT_DB::prepare("SELECT SQL_CACHE d_year, d_month, d_day FROM `##dates` WHERE d_fact=? AND d_gid=? AND d_file=? LIMIT 1")
			->execute(array($fact, $xref, $tree->tree_id))
			->fetchOneRow();
		if ($row) {
	...
	private function config() {
		...
		if(file_exists(WT_ROOT . 'gendex.txt')) {		
			$html .=  '<p>'.WT_I18N::translate('Click on the link below to view your GENDEX file.').'</p>'
					...
					. '<p>'.WT_I18N::translate('To tell search engines that a GENDEX file is available, you should submit the url above to the genealogical search engine of your choice.').'</p>'
					. '<p>'.WT_I18N::translate('The search engines below are known to accept the GENDEX textfile created by this module:').'</p>'
					. '<ul>'
					. ' <li><a href="http://www.gendexnetwork.org" target="_blank">The Gendex Network</a> ('.WT_I18N::translate('English/International').')</li>'
					. ' <li><a href="http://www.familytreeseeker.com/?l=en" target="_blank">Familytreeseeker</a> ('.WT_I18N::translate('English/International').')</li>'
					. '	<li><a href="http://www.stamboomzoeker.nl/?l=nl" target="_blank">Stamboomzoeker</a> ('.WT_I18N::translate('Dutch').')</li>'					
					. '</ul>'
					. '<p>'.WT_I18N::translate('You need this url (without the quotes) as general url to the individual pages when submitting to Stamboomzoeker or Familytreeseeker:'). ' “'.WT_SERVER_NAME.WT_SCRIPT_PATH.'individual.php?pid=”'
					. '</p><hr>'
